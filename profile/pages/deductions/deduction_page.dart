...
class _DeductionPageState extends ConsumerState<DeductionPage> {
  ...
  void _loadMoreListener() {
    if (_scrollController.position.pixels ==
      ...
      ref.read(deductionsProvider.notifier).loadMore();
    }
  }
          ...
          Expanded(
            child: Consumer(
              builder: (context, ref, child) {
                final deductionList = ref.watch(deductionsProvider);

                return deductionList.when(
                  data: (deductions) {
                    if (deductions.data.isEmpty) {
                      return Center(
                        child: Text(
                          LocaleKeys.no_deductions.tr(),
                    ...
                    return RefreshIndicator(
                      ...
                      child: ListView.separated(
                        ...
                        itemCount: deductions.data.length +
                            (deductions.hasNextPage ? 1 : 0),
                        itemBuilder: (context, index) {
                          if (index == deductions.data.length) {
                            return const Center(
                          ...
                          return GestureDetector(
                            onTap: () =>
                              ...
                              deductions.data[index].id.toString(),
                            )),
                            child: DeductionCard(
                              deductions.data[index],
                            ),
                          );
                        },
